use darklake_sdk::{
    amm::{CancelParams, FinalizeParams, SlashParams}, create_darklake_amm, get_pool_key, Amm, SettleParams, SwapMode, SwapParams
};
use solana_sdk::{
    pubkey::Pubkey,
...
async fn helper_order_finalize() -> Result<()> {
    println!("Darklake DEX SDK - Complete Example");
    println!("===================================");
    ...
    while !is_outdated {
        ...
        attempt_count += 1;
        is_outdated = darklake_amm.is_order_expired(&order_data.data, finalize_params.current_slot)?;
    ...
    let finalize_and_account_metas = darklake_amm.get_finalize_and_account_metas(&finalize_params)?;
    ...
    println!("   Settle transaction sent to network (signature: {})", _finalize_signature);
...
async fn main() -> Result<()> {
    Ok(helper_order_finalize().await?)
}
