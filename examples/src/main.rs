use darklake_sdk::{
    amm::{CancelParams, SlashParams}, create_darklake_amm, get_pool_key, Amm, SettleParams, SwapMode, SwapParams
};
use solana_sdk::{
    pubkey::Pubkey,
...
fn load_wallet_key() -> Result<Keypair> {
    let key_data = fs::read_to_string("key.json")
        .context("Failed to read key.json file")?;
    
    let key_bytes: Vec<u8> = serde_json::from_str(&key_data)
        .context("Failed to parse key.json as JSON array")?;
    
    if key_bytes.len() != 64 {
        bail!("Invalid key length: expected 64 bytes, got {}", key_bytes.len());
    }
...
async fn main() -> Result<()> {
    ...
    println!("   Supports exact out: {}", darklake_amm.supports_exact_out());
    ...
    for account_key in &accounts_to_update {
        if let Ok(account) = rpc_client.get_account(account_key) {
            account_map.insert(*account_key, darklake_sdk::amm::AccountData {
                data: account.data,
                owner: account.owner,
            });
        }
    ...
    let source_token_account = Pubkey::new_unique(); // Replace with actual token account
    let destination_token_account = Pubkey::new_unique(); // Replace with actual token account
    
    let salt = [1, 2, 3, 4, 5, 6, 7, 8];
    let min_out = 1_000_000_000;
    ...
    println!("   Discriminator: {:?}", swap_and_account_metas.discriminator);
    println!("   Amount in: {}", swap_and_account_metas.swap.amount_in);
    println!("   Is swap X to Y: {}", swap_and_account_metas.swap.is_swap_x_to_y);
    println!("   C min: {:?}", swap_and_account_metas.swap.c_min);
    println!("   Account metas count: {}", swap_and_account_metas.account_metas.len());
    
    // Step 7: Build and sign swap transaction
    println!("\nBuilding and signing swap transaction...");
    
    let mut data = swap_and_account_metas.discriminator.to_vec();
    data.extend_from_slice(&swap_and_account_metas.swap.amount_in.to_le_bytes());
    data.extend_from_slice(&[swap_and_account_metas.swap.is_swap_x_to_y as u8]);
    data.extend_from_slice(&swap_and_account_metas.swap.c_min);
    ...
    println!("✅ Swap transaction built and signed with wallet: {}", user_keypair.pubkey());
    println!("   Transaction signature: {}", swap_transaction.signatures[0]);
    ...
    println!("   Swap transaction sent to network (signature: {})", _swap_signature);
    ...
    let order_data = rpc_client.get_account(&order_key).context("Failed to get order data")?;
    ...
    while !is_outdated {
        ...
        println!("   Attempt {}: Order not yet outdated, waiting 1 seconds...", attempt_count);
    ...
    if is_outdated {
        ...
        println!("   Slash transaction sent to network (signature: {})", _slash_signature);
    ...
    if is_cancel {
        ...
        println!("   Cancel transaction sent to network (signature: {})", _cancel_signature);
    ...
    println!("   Discriminator: {:?}", settle_and_account_metas.discriminator);
    println!("   Proof A length: {} bytes", settle_and_account_metas.settle.proof_a.len());
    println!("   Proof B length: {} bytes", settle_and_account_metas.settle.proof_b.len());
    println!("   Proof C length: {} bytes", settle_and_account_metas.settle.proof_c.len());
    println!("   Public signals count: {}", settle_and_account_metas.settle.public_signals.len());
    println!("   Unwrap WSOL: {}", settle_and_account_metas.settle.unwrap_wsol);
    println!("   Account metas count: {}", settle_and_account_metas.account_metas.len());
    ...
    println!("✅ Settle transaction built and signed with wallet: {}", user_keypair.pubkey());
    println!("   Transaction signature: {}", settle_transaction.signatures[0]);
    
    // Note: In a real scenario, you would send this transaction
    let _settle_signature = rpc_client.send_and_confirm_transaction(&settle_transaction)?;
    println!("   Settle transaction sent to network (signature: {})", _settle_signature);
